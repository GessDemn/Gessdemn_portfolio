...
// Add an event listener to all "READ MORE" links
document.querySelectorAll('.learn-more').forEach((link) => {
    link.addEventListener('click', (event) => {
        event.preventDefault(); // Prevent the default link behavior
        
        // Find the parent `.single-schedule` container
        const container = link.closest('.single-schedule');
        
        // Toggle the 'expanded' class on the `.content-preview` within this container
        const content = container.querySelector('.content-preview');
        content.classList.toggle('expanded');

        // Change the link text based on the expanded state
        if (content.classList.contains('expanded')) {
            link.innerHTML = 'SHOW LESS <i class="fa fa-long-arrow-left"></i>';
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    readMoreBtns.forEach((btn) => {
        btn.addEventListener("click", () => {
            // Select the parent container of the button and toggle the "expanded" class
            const contentContainer = btn.closest(".content-container");

            contentContainer.classList.toggle("expanded");

            // Update the button text based on the state of the container
		...
		function isInViewport(el) {
		  ...
		  // Scroll event listener to apply animation
		  ...
		  document.addEventListener("DOMContentLoaded", () => {
			const sections = document.querySelectorAll(".section5");
